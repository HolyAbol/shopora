...
try{
   ...
   const result = await client.query("INSERT INTO addresses(user_id,address_detail,address_title,city,country_code,postal_code,province,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,now(),now()) RETURNING address_detail,address_title,city,country_code,postal_code,province",
...
async function changeAddress(req:Request,res:Response){
  ...
  const Details =addressDetails.safeParse(req.body)
    ...
    const {address_detail,address_title,city,country_code,postal_code,province}=Details.data
try{
    const result = await pool.query("UPDATE addresses SET address_detail=$1,address_title=$2,city=$3,country_code=$4,postal_code=$5,province=$6,updated_at=now() WHERE user_id=$7 AND deleted_at IS NULL RETURNING address_detail,address_title,city,country_code,postal_code,province",
...
async function deleteAddress(req:Request,res:Response){
    ...
    const user_id=req.user.user_id
try{
    const result = await pool.query("UPDATE addresses SET updated_at=now(),deleted_at=now() WHERE user_id=$1 AND deleted_at IS NULL RETURNING address_detail,address_title,city,country_code,postal_code,province,deleted_at",
        [user_id]
    )
    if(result.rowCount===0){
       return res.status(404).json({message:"create an address first"})
